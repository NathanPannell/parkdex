...
1. **Ship geometry by zoom and viewport.** Generate a versioned manifest and CDN-cacheable vector tiles from the existing canonical sources. Below zoom 7, show clusters and coarse territory or park outlines. At zooms 7 to 9, show simplified visible boundary tiles. At zoom 10 and above, load more detailed visible tiles. Preserve stable place IDs for visited styling and selection. Keep the small bounds index for fit-to-place, but remove the whole-province canonical boundary source from initial map loading. Fetch canonical geometry for a selected park on demand and package a compact offline fallback separately. Validate tile seams, island coverage, boundary hit testing, and selected or visited states across zoom transitions. MapLibre supports vector tile sources in its [official API](https://maplibre.org/maplibre-gl-js/docs/API/classes/VectorTileSource/).
...
Initial targets to validate on a midrange mobile browser: under 300 KB compressed map-owned geometry at first view, under 50 KB per visible geometry tile, and no app-owned viewport API query during a drag. MapLibre will request newly visible tiles as the map moves; measure tile requests per pan separately. These are proposed budgets, not measured performance results. Track first useful map render, bytes, memory, pan latency, and cache hits on staging before choosing final thresholds.
