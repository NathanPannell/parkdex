...
- Commit a durable deletion intent when a photo, visit, or account is deleted. The immediate delete path and hourly cron service schedule failures with capped exponential backoff so one failing object cannot starve newer removals; an operational orphan audit remains required before general availability.
