...
Parkdex uses short GitHub Actions merge deployments and long-lived provider resources. The only automatic deployment events are protected-branch merge pushes to `staging` and `main`. A workflow run validates one immutable Git SHA, stamps that SHA and a release ID on both Railway services, starts the API, worker, and Vercel uploads concurrently, and exits as soon as all three providers accept the requests. It never polls builds, waits for `/ready`, or runs browser smoke tests.
...
The API and worker both run the checksummed, advisory-locked migration command before starting. The lock wait is capped at five minutes. Both Railway environments must therefore give both services `DATABASE_URL_UNPOOLED`. Migrations must be additive and compatible with the old and new frontend, API, and worker while the providers converge.
...
Verify in Railway that both services show `python -m backend.app.migrate` as the pre-deploy command and that both have a `DATABASE_URL_UNPOOLED` variable before starting the release. The value must remain provider-managed and must not be copied into GitHub or logs.
...
3. From a separate clean checkout whose `HEAD` is the current `origin/staging`, build a full merge-candidate attestation and deploy the exact reviewed PR head to persistent staging:
   ...
   The local session must be authenticated to Railway and Vercel and must provide the exact project, service, environment, organization, and Vercel project identifiers. `RAILWAY_STAGING_ENVIRONMENT_ID` may be replaced by the existing `RAILWAY_BASE_ENVIRONMENT_ID`. The staging command changes only `APP_COMMIT_SHA` and `APP_RELEASE_ID`; it preserves stable provider configuration.
4. The local command waits until both Railway deployments, `/ready`, the worker marker, and the immutable Vercel deployment prove the exact SHA and release ID, then assigns `staging.parkdex.app`. Record the external release journal and exact provider identities in the task ledger.
...
## Production: merge overnight, verify, promote, stop

1. Open and review the `staging` to `main` pull request. Confirm staging is healthy and no staging or production release is still in flight.
2. During the requested off-hours window, the release agent records the current production revision and clicks merge.
...
4. The release agent waits locally for both Railway services, migrations, and the staged Vercel deployment to report the expected SHA and release ID. Test the immutable Vercel deployment before changing public domains when practical.
...
6. Verify frontend-to-API traffic, console and network output, `/ready`, and the worker catalogue marker. Record the evidence and stop once every check passes.
