...
class BoundaryRegistry:
    ...
    def recommend(self, sample: LocationSample, excluded_place_ids: Iterable[str] = ()) -> Candidate | None:
        ...
        point = Point(x, y)
        # Issue #84 defines a deliberate 10-50 m tolerance clamp so sub-10 m
        # fixes still absorb small discrepancies in published boundary lines.
        buffer_meters = min(
            MAX_ACCURACY_METERS,
            max(MIN_BOUNDARY_TOLERANCE_METERS, sample.accuracy_meters),
        )
