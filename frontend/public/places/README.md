...
These images depict the named park or a feature within its official boundary. Each source page identifies the location and publishes the image under the listed reusable license. Local copies were resized to a maximum width of 960 pixels and converted to WebP; the interface may crop them responsively.
...
Parks without a verified, freely licensed image use the explicit photo-unavailable treatment. To add one, confirm the exact park or an officially documented feature on the source page, preserve its creator and license links in `place-images.ts`, optimize a local WebP, and run the manifest tests.
