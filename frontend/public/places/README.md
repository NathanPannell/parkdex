...
These images depict the named park or a documented feature within its boundary. The coverage batches record their source review dates in the catalogue credit index and task ledger. Source pages and available API metadata are checked for the named location, creator, direct source image, and reusable license. When a source description names only a feature, the manifest links metadata or an official park page that confirms the feature is inside the park. The manifest records the location evidence and URL, alt text, source and direct image links, license link, and changes for every image.
...
The [expanded catalogue credit index](./CATALOGUE_CREDITS.md) lists photos by coverage batch, including the [2026-09-24 reviewed import](../../../docs/photo-import-2026-09-24.md). Each catalogue record includes a direct source image link and location evidence. Ordinary thumbnail contexts use the local text-free tree placeholder when a park has no verified, freely licensed image; group members omit missing media and place details elevate their factual metadata instead.
...
2. Confirm that the license permits local redistribution and presentation. Record the creator, source page, direct source image URL, license name and URL, location evidence, descriptive alt text, and every resize, conversion, or crop.
...
4. Add the primary record to `lib/place-images.catalogue.json` (or `lib/place-images.ts` for the base entries). Put additional reviewed photos for the same place in `lib/place-images.gallery.json`. Run `npm test -- place-images` from `frontend`. The integrity checks reject unknown IDs, duplicate or missing paths, oversized files, incomplete provenance, and unsupported licenses.
