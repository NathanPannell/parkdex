...
import { ArrowLeft, ArrowUpRight, Award, Check, ChevronDown, Compass, Heart, LandPlot, ListFilter, ListPlus, LocateFixed, LogIn, LogOut, MailCheck, Map as MapIcon, MapPin, Pencil, Plus, RotateCcw, Search, Trees, Trash2, UserRound, X } from "lucide-react";
...
export function ParkdexApp({ apiBaseUrl }: { apiBaseUrl: string }) {
  ...
  const selectedHasImage = Boolean(selected && getPlaceImage(selected.id));
  ...
  function navigate(next: View) {
    ...
    setSelectedId(null); setShowNearby(false); setShowFilters(false);
  ...
  return <main className="app-shell"><section className={`map-stage view-${view} ${groupMapMode ? "group-map-mode" : ""}`} aria-label="Parkdex explorer">
    <ParkMap places={selectedGroup ? selectedGroup.places : mapMode === "explored" ? places : mapFiltered} visited={visited} selectedId={selectedId} selectedIds={groupSelectedIds} resetViewRequest={resetViewRequest} onSelect={choosePlace} onBoundaryLoadState={setBoundaryLoadState} mode={selectedGroup ? "discover" : mapMode} currentLocation={location} />
    ...
    {selected && <article className={`place-sheet ${selectedHasImage ? "with-photo" : "without-photo"}`}><button className="sheet-close" onClick={() => { setSelectedId(null); setSearchExpanded(false); }} aria-label="Close place details"><X size={18} /></button><button className={`place-category category-${selected.category}`} onClick={() => openCollection(selected.category)}>{categoryLabels[selected.category]}<ArrowUpRight size={13} /></button><h2>{selected.name}</h2><div className="place-sheet-media">{selectedHasImage ? <PlaceImage place={selected} variant="card" /> : <div className="place-sheet-metadata" aria-label={`Details for ${selected.name}`}><PlaceMetadata place={selected} boundaryState={boundaryLoadState} openCollection={openCollection} /></div>}<div className="sheet-actions"><button aria-label={visited.has(selected.id) ? "Undo visited place" : "Mark as visited"} title={visited.has(selected.id) ? "Undo visited place" : "Mark as visited"} disabled={transitionBusy} className={`visit-button ${visited.has(selected.id) ? "is-visited" : ""}`} onClick={() => toggleSelected(selected)}>{visited.has(selected.id) ? <><RotateCcw size={19} /><span>Visited · undo</span></> : <><Check size={20} /><span>Mark as visited</span></>}</button>{authenticated && <GroupActions place={selected} groups={groupsState.groups} busy={groupsState.busy} onCreate={async (name, placeIds) => { const created = await groupsState.create(name, placeIds); groupsState.selectGroup(null); return created; }} onAddPlace={groupsState.addPlace} onRemovePlace={groupsState.removePlace} />}</div></div>{selectedHasImage && <PlaceMetadata place={selected} boundaryState={boundaryLoadState} openCollection={openCollection} />}<p className="place-description">{selected.description}</p></article>}
...
function PlaceMetadata({ place, boundaryState, openCollection }: { place: Place; boundaryState: BoundaryLoadState; openCollection: (category?: PlaceCategory, authority?: string) => void }) { return <><p className="place-region"><MapPin size={15} />{place.region}</p><button className="place-collection-link" onClick={() => openCollection(undefined, authorityForPlace(place))}>{authorityForPlace(place)}<ArrowUpRight size={13} /></button><PlaceProvenance place={place} boundaryState={boundaryState} /></>; }
