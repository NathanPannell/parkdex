...
type View = "map" | "collection" | "trips" | "badges" | "account";
...
export function ParkdexApp({ apiBaseUrl }: { apiBaseUrl: string }) {
  ...
  const tripsState = useTrips({ apiBaseUrl, authenticated: authenticated && !recoveryActive, identityKey: account?.id ?? "", places, request: authenticatedRequest });
  const selectedTrip = tripsState.trips.find((trip) => trip.id === tripsState.selectedTripId) ?? null;
  const tripSelectedIds = useMemo(() => new Set(selectedTrip?.places.map((place) => place.id) ?? []), [selectedTrip]);
  ...
  function navigate(next: View) { setSelectedId(null); setShowNearby(false); setShowFilters(false); setView((current) => ((next === "collection" || next === "trips" || next === "badges") && current === next ? "map" : next)); }
  ...
  return <main className="app-shell"><section className={`map-stage view-${view}`} aria-label="Parkdex explorer">
    <ParkMap places={selectedTrip || mapMode === "explored" ? places : mapFiltered} visited={visited} selectedId={selectedId} selectedIds={tripSelectedIds} onSelect={choosePlace} onBoundaryLoadState={setBoundaryLoadState} mode={selectedTrip ? "discover" : mapMode} currentLocation={location} />
...
function GroupActions({ place, groups, busy, onCreate, onAddPlace, onRemovePlace }: { place: Place; groups: import("@/lib/trips").Trip[]; busy: boolean; onCreate: (name: string, placeIds: string[]) => Promise<import("@/lib/trips").Trip | null>; onAddPlace: (groupId: string, placeId: string) => Promise<void>; onRemovePlace: (groupId: string, placeId: string) => Promise<void> }) {
...
type TripsViewProps = {
  ...
  onClear: () => void;
  onCreate: (name: string, placeIds: string[]) => Promise<import("@/lib/trips").Trip | null>;
...
function TripsView({ places, trips, selectedTripId, loading, error, busy, onRetry, onSelect, onClear, onCreate, onRename, onDelete, onAddPlace, onRemovePlace, onViewMap }: TripsViewProps) {
  ...
  const wishlist = trips.find((trip) => trip.isWishlist);
  const ordinaryTrips = trips.filter((trip) => !trip.isWishlist);
  return <div className="trips-view"><div className="panel-heading trips-heading"><div><h2>Groups</h2><p>Gather places for the day ahead.</p></div><button className="primary-action trips-create-toggle" onClick={() => { setShowCreate((current) => !current); setActionError(""); }} aria-expanded={showCreate}><Plus size={18} />New group</button></div>
    {error && <div className="trips-alert" role="alert"><span>{error}</span><button onClick={() => void onRetry()} disabled={loading}>Try again</button></div>}
    {showCreate && <form className="trip-create" onSubmit={(event) => void submitCreate(event)}><h3>Plan a new group</h3><label>Group name<input aria-label="Group name" value={name} onChange={(event) => setName(event.target.value)} placeholder="West coast weekend" maxLength={80} autoFocus /></label><p>Choose places to include</p><div className="trip-place-picker">{places.map((place) => <label key={place.id} className={selectedPlaces.has(place.id) ? "chosen" : ""}><input type="checkbox" checked={selectedPlaces.has(place.id)} onChange={() => togglePlace(place.id)} /><span>{place.name}</span><small>{categoryLabels[place.category]} · {place.region}</small></label>)}</div>{actionError && <p className="trips-inline-error" role="alert">{actionError}</p>}<div className="trip-form-actions"><button type="button" className="secondary-action" onClick={() => setShowCreate(false)}>Cancel</button><button className="primary-action" disabled={busy}>{busy ? "Saving…" : "Save group"}</button></div></form>}
    {loading && <div className="trips-state" role="status"><span className="state-pulse" />Loading your groups…</div>}
    {!loading && !error && !ordinaryTrips.length && !wishlist && !showCreate && <div className="trips-empty"><MapPin size={34} /><h3>No groups yet</h3><p>Make a small route from places you already know, then keep the whole day together on the map.</p><button className="primary-action" onClick={() => setShowCreate(true)}><Plus size={18} />Create your first group</button></div>}
    {!loading && trips.length > 0 && <div className="trip-layout"><div className="trip-list" aria-label="Your groups">{[wishlist, ...ordinaryTrips].filter((trip): trip is import("@/lib/trips").Trip => Boolean(trip)).map((trip) => <article className={`trip-item ${trip.id === selectedTripId ? "active" : ""}`} key={trip.id}><button className="trip-item-select" onClick={() => onSelect(trip.id === selectedTripId ? null : trip.id)} aria-pressed={trip.id === selectedTripId}><span><strong>{trip.isWishlist ? "Wishlist" : trip.name}</strong><small>{trip.places.length} {trip.places.length === 1 ? "place" : "places"}{trip.isWishlist ? " · saved for later" : ""}</small></span><ChevronDown size={17} /></button></article>)}</div>{selected && <TripDetail trip={selected} places={places} busy={busy} editingId={editingId} editingName={editingName} actionError={actionError} setEditingId={setEditingId} setEditingName={setEditingName} onRename={saveRename} onDelete={() => void removeTrip(selected.id)} onAddPlace={onAddPlace} onRemovePlace={onRemovePlace} onViewMap={onViewMap} onClear={onClear} />}</div>}
  </div>;
}

function TripDetail({ trip, places, busy, editingId, editingName, actionError, setEditingId, setEditingName, onRename, onDelete, onAddPlace, onRemovePlace, onViewMap, onClear }: { trip: import("@/lib/trips").Trip; places: Place[]; busy: boolean; editingId: string | null; editingName: string; actionError: string; setEditingId: (value: string | null) => void; setEditingName: (value: string) => void; onRename: (event: React.FormEvent, id: string) => Promise<void>; onDelete: () => void; onAddPlace: (tripId: string, placeId: string) => Promise<void>; onRemovePlace: (tripId: string, placeId: string) => Promise<void>; onViewMap: () => void; onClear: () => void }) {
  const [addOpen, setAddOpen] = useState(false);
  const members = new Set(trip.places.map((place) => place.id));
  const available = places.filter((place) => !members.has(place.id));
  return <article className="trip-detail"><header>{editingId === trip.id ? <form className="trip-rename" onSubmit={(event) => void onRename(event, trip.id)}><label className="sr-only" htmlFor="trip-rename-input">Group name</label><input id="trip-rename-input" value={editingName} onChange={(event) => setEditingName(event.target.value)} maxLength={80} aria-describedby={actionError ? "trip-rename-error" : undefined} aria-invalid={Boolean(actionError)} autoFocus /><button aria-label="Save group name" disabled={busy}><Check size={17} /></button>{actionError && <p id="trip-rename-error" className="trips-inline-error" role="alert">{actionError}</p>}</form> : <><div><h3>{trip.isWishlist ? "Wishlist" : trip.name}</h3><p>{trip.places.length} places · shown together on the map</p></div>{!trip.isWishlist && <div className="trip-detail-actions"><button onClick={() => { setEditingId(trip.id); setEditingName(trip.name); }} aria-label={`Rename ${trip.name}`}><Pencil size={16} /></button><button onClick={onDelete} aria-label={`Delete ${trip.name}`}><Trash2 size={16} /></button></div>}</>}</header><button className="primary-action trip-map-action" onClick={onViewMap} disabled={!trip.places.length}><MapIcon size={17} />Show all on map</button>{trip.places.length > 0 && <button className="trip-clear-action" onClick={onClear}>Clear map selection</button>}<div className="trip-members" aria-label={`${trip.isWishlist ? "Wishlist" : trip.name} places`}>{trip.places.length ? trip.places.map((place) => <div className="trip-member" key={place.id}><PlaceImage place={place} variant="thumbnail" /><span><strong>{place.name}</strong><small>{categoryLabels[place.category]} · {place.region}</small></span><button onClick={() => void onRemovePlace(trip.id, place.id)} aria-label={`Remove ${place.name} from ${trip.isWishlist ? "Wishlist" : trip.name}`} disabled={busy}><X size={16} /></button></div>) : <p className="trip-no-places">This group is empty. Add a place below.</p>}</div>{available.length > 0 && <div className="trip-add"><button className="secondary-action" onClick={() => setAddOpen((current) => !current)} aria-expanded={addOpen}><Plus size={17} />Add places</button>{addOpen && <div className="trip-add-menu">{available.map((place) => <button key={place.id} onClick={() => void onAddPlace(trip.id, place.id)} disabled={busy}><span>{place.name}</span><small>{place.region}</small></button>)}</div>}</div>}</article>;
}
