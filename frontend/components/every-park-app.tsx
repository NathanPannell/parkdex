...
export function ParkdexApp({ apiBaseUrl, googleAuthAllowed = true, geolocationAllowed = true, automaticLocationAllowed = false }: { apiBaseUrl: string; googleAuthAllowed?: boolean; geolocationAllowed?: boolean; automaticLocationAllowed?: boolean }) {
  ...
  useEffect(() => {
    function restoreHistory() { setShowNearby(false); setShowFilters(false); setSearchExpanded(false); setNavigationNotice(""); selectGroup(null); }
  ...
  const choosePlace = useCallback((id: string) => { updateNavigation({ mapMode: modeForSelection(mapMode, visited, id), selectedId: id, view: "map" }, "push"); setNavigationNotice(""); setShowNearby(false); setShowFilters(false); }, [visited, mapMode, updateNavigation]);
  ...
  function openCollection(category?: PlaceCategory, authority?: string) {
    ...
    groupsState.selectGroup(null); setShowFilters(false);
  }
...
function PlaceDetail({ place, visit, visited, busy, authenticated, claimsAvailable, legacyVisitCreationAvailable, recommendClaim, createClaim, uploadPhoto, loadPhoto, removePhoto, ownerKey, onClaimed, onOpenPlace, groupsState, boundaryState, onClose, onToggle, openCollection }: {
  ...
  return <article className={`place-sheet ${hasImage ? "with-photo" : "without-photo"}`} aria-labelledby="place-detail-title">
    ...
    <div className="place-sheet-content">
      <div className="place-sheet-media">{hasImage ? <PlaceImage place={place} variant="card" /> : <div className="place-sheet-metadata" aria-label={`Details for ${place.name}`}><PlaceMetadata place={place} boundaryState={boundaryState} openCollection={openCollection} /></div>}
        <div className="sheet-actions">{(visited || legacyVisitCreationAvailable) && <button aria-label={visited ? "Undo visited place" : "Mark as visited"} title={visited ? "Undo visited place" : "Mark as visited"} aria-pressed={visited} disabled={busy} className={`visit-button ${visited ? "is-visited" : ""}`} onClick={onToggle}>{visited ? <RotateCcw size={20} /> : <Check size={20} />}<span className="action-hint">{visited ? "Undo visited" : "Mark visited"}</span></button>}
...
function GroupSyncNotice({ offline, syncStatus, syncMessage, pendingMemberships, onRetry }: Pick<GroupsViewProps, "offline" | "syncStatus" | "syncMessage" | "pendingMemberships" | "onRetry">) {
  ...
  return <div className={`groups-sync-state ${syncStatus}`} role="status" aria-live="polite"><span><strong>{title}</strong><small>{detail}</small></span>{!offline && (pendingMemberships > 0 || syncStatus === "error") && <button onClick={() => void onRetry()}>Try sync</button>}</div>;
}
...
type AccountViewProps = {
  ...
  badges: Achievement[]; places: Place[]; visits: Record<string, Visit>; visitTimestamps: Record<string, string>; loadPhoto?: (placeId: string) => Promise<Blob>; removePhoto?: (placeId: string) => Promise<void>; photoOwnerKey?: string; choosePlace: (id: string) => void;
};
...
function AccountView({ apiBaseUrl, googleAuthAllowed, account, authenticated, sessionAuthenticated, loading, busy, guestProgressAvailable, onAuth, onGoogleAuth, onExitRecovery, onRequestVerification, onConfirmVerification, onImport, onLogout, onReset, badges, places, visits, visitTimestamps, loadPhoto, removePhoto, photoOwnerKey, choosePlace }: AccountViewProps) {
...
function ResetConfirmation({ busy, onCancel, onConfirm }: { busy: boolean; onCancel: () => void; onConfirm: () => Promise<void> }) { const ref = useDialogFocus(onCancel); const [error, setError] = useState(""); return <div ref={ref as React.RefObject<HTMLDivElement>} className="reset-backdrop" role="dialog" aria-modal="true" aria-labelledby="reset-title" onClick={onCancel}><section className="reset-dialog" onClick={(event) => event.stopPropagation()}><div className="reset-icon"><Trash2 size={24} /></div><h2 id="reset-title">Reset all progress?</h2><p>This removes every visited place, completed trail, earned badge, group, and Wishlist save from your account. Your empty Wishlist will be ready to use again. This cannot be undone.</p>{error && <p className="reset-error" role="alert">{error}</p>}<div><button disabled={busy} onClick={onCancel}>Keep my progress</button><button disabled={busy} onClick={() => { setError(""); void onConfirm().catch((caught) => setError(caught instanceof Error ? caught.message : "Could not reset progress.")); }}>{busy ? "Resetting…" : "Reset everything"}</button></div></section></div>; }
