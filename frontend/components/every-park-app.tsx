...
import { ArrowUpRight, Award, Check, ChevronDown, Compass, KeyRound, LandPlot, ListFilter, LocateFixed, LogIn, LogOut, MailCheck, Map as MapIcon, MapPin, RotateCcw, Search, Trees, Trash2, UserRound, X } from "lucide-react";
...
import { confirmPasswordReset, loadAuthConfig, requestGoogleAuthorization, requestPasswordReset, type Account, type AuthConfig, type Visit } from "@/lib/account";
...
import { getPlatformStorage } from "@/lib/platform-storage";
import type { ClaimConfirmation } from "@/lib/claims-client";
...
export function ParkdexApp({ apiBaseUrl }: { apiBaseUrl: string }) {
  ...
  const photoOwnerKey = authenticated ? `account:${account?.id ?? "loading"}` : "guest";
  ...
  useEffect(() => {
    function nativeBack(event: Event) {
      ...
      else if (celebrationBadges.length) setCelebrationBadges((current) => current.slice(1));
      else if (showFilters) setShowFilters(false);
      else if (showNearby) setShowNearby(false);
      else if (selectedId) setSelectedId(null);
      else if (view !== "map") setView("map");
      else consumed = false;
      if (consumed && event.cancelable) event.preventDefault();
    }
    window.addEventListener("parkdex:back", nativeBack);
    return () => window.removeEventListener("parkdex:back", nativeBack);
  }, [celebrationBadges.length, selectedId, showFilters, showNearby, view]);
  ...
  function celebrateClaim(confirmation: ClaimConfirmation) {
    const nextVisited = new Set(visited).add(confirmation.placeId), nextTimestamps = { ...visitTimestamps, [confirmation.placeId]: confirmation.visitedAt };
    const nextBadges = achievements({ places, visited: nextVisited, visitTimestamps: nextTimestamps });
    const earnedIds = new Set(newlyEarnedAchievementIds(badgeList, nextBadges));
    setCelebrationBadges(nextBadges.filter((badge) => earnedIds.has(badge.id)));
  }
  ...
  return <main className="app-shell"><section className={`map-stage view-${view}`} aria-label="Parkdex explorer">
    <ParkMap places={mapMode === "explored" ? places : mapFiltered} visited={visited} selectedId={selectedId} onSelect={choosePlace} onBoundaryLoadState={setBoundaryLoadState} mode={mapMode} currentLocation={location} />
...
function AccountView({ apiBaseUrl, account, authenticated, sessionAuthenticated, loading, busy, guestProgressAvailable, onAuth, onGoogleAuth, onChangePassword, onRequestVerification, onConfirmVerification, onImport, onLogout, onReset, badges, places, visitTimestamps, visitMetadata, loadPhoto, removePhoto, photoOwnerKey, choosePlace }: AccountViewProps) {
  ...
  useEffect(() => {
    ...
    async function completeCallback(callback: { code?: string; state?: string; error?: string }) {
      ...
      if (callback.error || callback.code) {
        const expectedState = await storage.getItem(GOOGLE_STATE_KEY);
        if (!callback.state || !expectedState || callback.state !== expectedState) throw new Error("Google sign-in could not be verified. Please start again.");
      }
      if (callback.error) {
        await Promise.all([storage.removeItem(GOOGLE_VERIFIER_KEY), storage.removeItem(GOOGLE_STATE_KEY)]); cleanAuthParams(["error", "error_description", "state"]);
        throw new Error(callback.error === "access_denied" ? "Google sign-in was cancelled. You can try again." : "Google could not complete sign-in. Please try again.");
      }
      if (callback.code && callback.state) {
        const verifier = await storage.getItem(GOOGLE_VERIFIER_KEY);
        if (!verifier) throw new Error("Google sign-in expired. Please try again.");
        await Promise.all([storage.removeItem(GOOGLE_VERIFIER_KEY), storage.removeItem(GOOGLE_STATE_KEY)]); cleanAuthParams(["code", "state"]);
        await onGoogleAuth(callback.code, callback.state, verifier);
    ...
    function handle(callback: { code?: string; state?: string; error?: string }) {
      if (callbackHandled.current && !verificationToken) return;
      callbackHandled.current = true; setFormBusy(true); setError("");
      void completeCallback(callback).catch((caught) => setError(caught instanceof Error ? caught.message : "Could not complete this account link.")).finally(() => setFormBusy(false));
    }
  ...
  async function startGoogle() { setFormBusy(true); setError(""); try { const verifier = randomVerifier(); const storage = await getPlatformStorage(); const url = await requestGoogleAuthorization(apiBaseUrl, await sha256Challenge(verifier)); const state = new URL(url, window.location.origin).searchParams.get("state"); if (!state) throw new Error("Google sign-in could not be prepared. Please try again."); await Promise.all([storage.setItem(GOOGLE_VERIFIER_KEY, verifier), storage.setItem(GOOGLE_STATE_KEY, state)]); const external = getNativeCapabilities().openExternalAuth; if (external) await external(url); else window.location.assign(url); } catch (caught) { setError(caught instanceof Error ? caught.message : "Could not start Google sign-in."); setFormBusy(false); } }
