...
import { ArrowLeft, ArrowUpRight, Award, Check, ChevronDown, ChevronRight, Circle, Compass, Heart, LandPlot, ListFilter, ListPlus, LocateFixed, LogIn, LogOut, MailCheck, Map as MapIcon, MapPin, Pencil, Plus, RotateCcw, Search, Trees, Trash2, UserRound, X } from "lucide-react";
...
export function ParkdexApp({ apiBaseUrl, googleAuthAllowed = true, geolocationAllowed = true }: { apiBaseUrl: string; googleAuthAllowed?: boolean; geolocationAllowed?: boolean }) {
  ...
  const mapPlaces = useMemo(() => {
    const candidates = selectedGroup ? selectedGroup.places : mapMode === "explored" ? places : mapFiltered;
  ...
  const choosePlace = useCallback((id: string) => {
    ...
    updateNavigation({ mapMode: modeForSelection(mapMode, visited, id), selectedId: id, view: "map" }, "push");
    rememberGroupNavigation(null, 0, groupNavigationAccountId);
    setNavigationNotice(""); setShowNearby(false); setShowFilters(false);
  }, [mapMode, rememberDepartingGroup, updateNavigation, visited, groupNavigationAccountId]);
  ...
  function viewSelectedGroupOnMap() {
    ...
    updateNavigation({ view: "map", selectedId: null }, "push");
  }
  ...
  return <main className="app-shell"><div className="skip-controls"><button onClick={focusNavigation}>Skip to navigation</button><button onClick={focusContent}>Skip to content</button></div><section ref={mapStageRef} className={`map-stage view-${view} ${groupMapMode ? "group-map-mode" : ""}`} aria-label="Parkdex explorer">
    ...
    {view === "map" && !groupMapMode && !selected && <><div className={`map-utility ${searchExpanded ? "search-open" : ""}`} role="toolbar" aria-label="Map utilities"><div className="map-mode-switch"><button className={mapMode === "explored" ? "active" : ""} onClick={() => { setMapMode("explored"); setShowNearby(false); }}><Trees size={16} /><span>My map</span></button><button className={mapMode === "discover" ? "active" : ""} onClick={() => setMapMode("discover")}><Compass size={16} /><span>Find places</span></button>{geolocationAllowed && <button className="locate-button" onClick={requestLocation} aria-label="Show my current location"><LocateFixed size={19} className={locationStatus === "locating" ? "spin" : ""} /></button>}</div><div className={`search-dock ${searchExpanded ? "expanded" : "collapsed"}`}>
        ...
        {searchExpanded && <input ref={searchInputRef} aria-label="Search places" value={mapSearchDraft} onFocus={() => setShowFilters(false)} onClick={() => setShowFilters(false)} onChange={(event) => setMapSearchDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") applyMapSearch(); if (event.key === "Escape" && !mapSearchDraft) setSearchExpanded(false); }} placeholder="Find a park or region" />}
      ...
      {!searchExpanded && mapMode === "discover" && (mapSearch.trim() || mapCategories.size > 0) && <div className="applied-map-search"><div role="status"><strong>{mapFiltered.length ? `${mapFiltered.length} ${mapFiltered.length === 1 ? "place" : "places"} found` : "No places match"}</strong><span>{[mapSearch.trim() ? `“${mapSearch.trim()}”` : "", ...[...mapCategories].map((category) => categoryLabels[category])].filter(Boolean).join(" · ")}</span></div><button onClick={resetMapFilters} aria-label="Clear map search and filters"><X size={17} /><span>Clear</span></button></div>}
      {mapMode === "explored" && visited.size === 0 && <div className="first-adventure"><strong>Your next adventure starts here.</strong><span>{authenticated ? claimsAvailable ? "Find a place, then confirm your visit while you’re there." : legacyVisitCreationAvailable ? "Find a place, then mark it visited to begin your map." : "Reconnect to check visit confirmation support." : "Find a place to explore, then sign in to confirm visits."}</span><button onClick={() => setMapMode("discover")}><Compass size={17} />Find your first place</button></div>}
    ...
    {view !== "map" && <section key={`${view}-${viewRevision}`} className={`feature-panel feature-${view}`}><button className="content-skip" onClick={focusNavigation}>Skip to navigation</button><div id="primary-content" tabIndex={-1} className="primary-content-target" aria-label={view === "collection" ? "Places" : view === "badges" ? "Badges" : view === "groups" ? "Groups" : "Account"} />{view === "collection" && <CollectionView groups={groups} places={places} visited={visited} search={collectionSearch} setSearch={setCollectionSearch} selectedCategories={collectionCategories} authorities={collectionAuthorities} visitFilter={collectionVisitFilter} setVisitFilter={setCollectionVisitFilter} toggleCategory={(value) => toggleSet(setCollectionCategories, value)} resetFilters={resetCollectionFilters} choosePlace={choosePlace} />}{view === "groups" && authenticated && !recoveryActive && <GroupsView places={places} groups={groupsState.groups} selectedGroupId={groupsState.selectedGroupId} loading={groupsState.loading} error={groupsState.error} busy={groupsState.busy} offline={groupsState.offline} syncStatus={groupsState.syncStatus} syncMessage={groupsState.syncMessage} pendingMemberships={groupsState.pendingMemberships} onRetry={groupsState.retry} onSelect={groupsState.selectGroup} onClear={() => groupsState.selectGroup(null)} onCreate={groupsState.create} onRename={groupsState.rename} onDelete={groupsState.remove} onAddPlace={groupsState.addPlace} onRemovePlace={groupsState.removePlace} onViewMap={viewSelectedGroupOnMap} onOpenPlace={openGroupMember} />}{view === "badges" && <BadgesView badges={badgeList} earned={earnedBadges} places={places} visited={visited} onOpenPlace={choosePlace} />}{(view === "account" || (view === "groups" && (!authenticated || recoveryActive))) && <AccountView apiBaseUrl={apiBaseUrl} googleAuthAllowed={googleAuthAllowed} account={account} authenticated={authenticated && !recoveryActive} sessionAuthenticated={authenticated} loading={loading} busy={transitionBusy} guestProgressAvailable={guestProgressAvailable} onAuth={completeAuth} onGoogleAuth={authenticateWithGoogle} onExitRecovery={() => setRecoveryActive(false)} onRequestVerification={requestEmailVerification} onConfirmVerification={confirmEmailVerification} onImport={importGuest} onLogout={logoutAndClearGroupHistory} onReset={async () => { await resetProgress(); setCelebrationBadges([]); setProgressHoldCount(null); await groupsState.refreshAfterReset(); }} badges={badgeList} places={places.filter((place) => visited.has(place.id))} allPlaces={places} visits={visits} visitTimestamps={visitTimestamps} loadPhoto={loadVisitPhoto} removePhoto={removeVisitPhoto} photoOwnerKey={photoOwnerKey} choosePlace={choosePlace} />}</section>}
...
function PlaceDetail({ place, visit, visited, busy, authenticated, claimsAvailable, legacyVisitCreationAvailable, recommendClaim, createClaim, uploadPhoto, loadPhoto, removePhoto, ownerKey, onClaimed, onOpenPlace, groupsState, boundaryState, onClose, onToggle, openCollection }: {
  place: Place;
  visit?: Visit;
  visited: boolean;
  busy: boolean;
  ...
  const hasImage = Boolean(getPlaceImage(place.id));
  ...
  return <article className={`place-sheet ${hasImage ? "with-photo" : "without-photo"} ${authenticated ? "signed-in" : "guest place-sheet-guest"}`} data-authenticated={authenticated ? "true" : "false"} aria-labelledby="place-detail-title">
    ...
    <div className="place-sheet-content">
      {(claimsAvailable || (!authenticated && typeof recommendClaim === "function")) && recommendClaim && createClaim && uploadPhoto && loadPhoto && removePhoto && <div className="place-primary-action-panel" data-primary-action="visit"><ClaimVisitPanel authenticated={authenticated} place={place} visit={visit} busy={busy} recommendClaim={recommendClaim} createClaim={createClaim} uploadPhoto={uploadPhoto} loadPhoto={loadPhoto} removePhoto={removePhoto} onClaimed={onClaimed} ownerKey={ownerKey} onOpenPlace={onOpenPlace} placeNameForId={(placeId) => groupsState.groups.flatMap((group) => group.places).find((candidate) => candidate.id === placeId)?.name} /></div>}
      <div className="place-sheet-media">{hasImage ? <PlaceImage place={place} variant="card" /> : <div className="place-sheet-metadata" aria-label={`Details for ${place.name}`}><PlaceMetadata place={place} boundaryState={boundaryState} openCollection={openCollection} /></div>}
...
function PlaceMetadata({ place, boundaryState, openCollection }: { place: Place; boundaryState: BoundaryLoadState; openCollection: (category?: PlaceCategory, authority?: string) => void }) { return <><p className="place-region"><MapPin size={15} />{place.region}</p><button className="place-collection-link" onClick={() => openCollection(undefined, authorityForPlace(place))}>Browse {collectionTitle(authorityForPlace(place))}<ChevronRight size={15} /></button>{getVisitorInformation(place.id) && <a className="official-visitor-link" href={getVisitorInformation(place.id)!.url} target="_blank" rel="noreferrer">Official visitor information<ArrowUpRight size={15} /></a>}<PlaceProvenance place={place} boundaryState={boundaryState} /></>; }
...
function CollectionView({ groups, places, visited, search, setSearch, selectedCategories, authorities, visitFilter, setVisitFilter, toggleCategory, resetFilters, choosePlace }: { groups: ReturnType<typeof groupByAuthority>; places: Place[]; visited: Set<string>; search: string; setSearch: (v: string) => void; selectedCategories: Set<PlaceCategory>; authorities: Set<string>; visitFilter: VisitFilter; setVisitFilter: (v: VisitFilter) => void; toggleCategory: (v: PlaceCategory) => void; resetFilters: () => void; choosePlace: (id: string) => void }) {
  ...
  return <div className={`collection-view search-open ${hasQuery ? "has-query" : ""}`} data-query-active={hasQuery ? "true" : "false"} data-filter-count={activeFilterLabels.length}><div className="collection-scroll"><div className="panel-heading collection-heading" data-query-heading={hasQuery ? "compact" : "full"}><div><h2>Places</h2><p>A field index for every corner of the island.</p></div></div><CategoryProgress places={places} visited={visited} /><div className="filter-block" role="group" aria-label="Visit status"><span>Show</span><div className="segmented">{(["all", "unseen", "visited"] as VisitFilter[]).map((value) => <button key={value} className={visitFilter === value ? "active" : ""} onClick={() => setVisitFilter(value)} aria-pressed={visitFilter === value}>{value}</button>)}</div></div><div className="chip-row category-chips" role="group" aria-label="Place categories">{categories.map((category) => <button key={category} className={`category-${category} ${selectedCategories.has(category) ? "active selected" : ""}`} onClick={() => toggleCategory(category)} aria-pressed={selectedCategories.has(category)}>{categoryLabels[category]}</button>)}</div>{hasQuery && hasActiveFilters && <div className="collection-filter-summary" role="status"><span><strong>Filters:</strong> {activeFilterLabels.join(" · ")}</span><button onClick={resetFilters} aria-label="Clear active Places filters">Clear filters</button></div>}{groups.length === 0 ? <div className="empty-state"><Compass size={30} /><strong>No places match.</strong><button onClick={resetFilters}>Clear filters</button></div> : <div className="authority-list">{groups.map((group) => <details key={group.authority} open={Boolean(search) || authorities.has(group.authority)}><summary><span>{collectionTitle(group.authority)}{/\([^)]+\)$/.test(group.authority) && <small>{group.authority.replace(/\s*\([^)]+\)$/, "")}</small>}</span><b>{group.places.filter((place) => visited.has(place.id)).length}/{group.places.length}</b><ChevronDown size={17} /></summary><div>{group.places.map((place) => <PlaceListRow key={place.id} place={place} visited={visited.has(place.id)} onSelect={() => choosePlace(place.id)} />)}</div></details>)}</div>}</div><div className="collection-search-dock"><Search className="collection-search-icon" size={20} aria-hidden="true" /><input aria-label="Search collection" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Find a park or region" />{search && <button className="collection-search-clear" onClick={() => setSearch("")} aria-label="Clear collection search"><X size={17} /></button>}</div></div>;
}
...
function GroupPickerModal({ place, groups, membership, busy, offline, error, onError, onClose, onCreate, onAddPlace }: { place: Place; groups: import("@/lib/groups").Group[]; membership: Set<string>; busy: boolean; offline: boolean; error: string; onError: (value: string) => void; onClose: () => void; onCreate: (name: string, placeIds: string[]) => Promise<import("@/lib/groups").Group | null>; onAddPlace: (groupId: string, placeId: string) => Promise<void> }) {
  ...
  async function create(event: React.FormEvent) { event.preventDefault(); if (offline) { onError("Reconnect to create a new group."); return; } if (!name.trim()) { onError("Name your group first."); return; } onError(""); try { await onCreate(name.trim(), [place.id]); onClose(); } catch (caught) { onError(caught instanceof Error ? caught.message : "Could not create this group."); } }
  const modal = <div ref={ref as React.RefObject<HTMLDivElement>} className="group-picker-backdrop" role="dialog" aria-modal="true" aria-labelledby="group-picker-title" onClick={onClose}><section className="group-picker-modal" onClick={(event) => event.stopPropagation()}><header><div><p>Add this place</p><h2 id="group-picker-title">{place.name}</h2></div><button onClick={onClose} aria-label="Close group picker"><X size={21} /></button></header><div className="group-picker-options">{ordinaryGroups.length ? <><p className="group-picker-label">Your groups</p>{ordinaryGroups.map((group) => { const included = membership.has(group.id); const content = <><span><strong>{group.name}</strong><small>{included ? "Already added" : `${group.places.length} ${group.places.length === 1 ? "place" : "places"}`}</small></span><Circle size={20} fill={included ? "currentColor" : "none"} aria-hidden="true" /></>; return included ? <div key={group.id} className="group-picker-option is-member">{content}</div> : <button key={group.id} className="group-picker-option" onClick={() => void add(group.id)} disabled={busy} aria-label={`Add ${place.name} to ${group.name}`}>{content}</button>; })}</> : <p className="group-picker-empty">Create a group to start a new collection.</p>}<form className="group-picker-create" onSubmit={(event) => void create(event)}><label htmlFor="group-picker-new-name">Create a new group</label><div><input id="group-picker-new-name" value={name} onChange={(event) => setName(event.target.value)} placeholder="Coastal weekends" maxLength={80} /><button type="submit" aria-label="Create group" disabled={busy || offline} title={offline ? "Reconnect to create a group" : undefined}><Plus size={18} /></button></div>{offline && <small>Reconnect to create a group. You can still save this place to an existing group.</small>}</form>{error && <p className="groups-inline-error" role="alert">{error}</p>}</div></section></div>;
...
function GroupSyncNotice({ offline, syncStatus, syncMessage, pendingMemberships, onRetry }: Pick<GroupsViewProps, "offline" | "syncStatus" | "syncMessage" | "pendingMemberships" | "onRetry">) {
  if (syncStatus === "idle" && !syncMessage) return null;
  const title = offline ? "Working offline" : syncStatus === "syncing" ? "Syncing groups" : "Groups need attention";
  const count = `${pendingMemberships} ${pendingMemberships === 1 ? "change" : "changes"}`;
  const detail = syncMessage || (pendingMemberships ? `${count} waiting to sync.` : "Checking for the latest groups.");
  const canRetry = offline || pendingMemberships > 0 || syncStatus === "error";
  return <div className={`groups-sync-state ${syncStatus}`} role="status" aria-live="polite"><span><strong>{title}</strong><small>{detail}</small></span>{canRetry && <button onClick={() => void onRetry()}>Try again</button>}</div>;
}

function GroupsView({ places, groups, selectedGroupId, loading, error, busy, offline, syncStatus, syncMessage, pendingMemberships, onRetry, onSelect, onClear, onCreate, onRename, onDelete, onAddPlace, onRemovePlace, onViewMap, onOpenPlace }: GroupsViewProps) {
  const [name, setName] = useState("");
  ...
  async function submitCreate(event: React.FormEvent) {
    ...
    if (offline) { setActionError("Reconnect to create a new group."); return; }
  ...
  async function saveRename(event: React.FormEvent, id: string) {
    event.preventDefault();
    if (offline) { setActionError("Reconnect to rename this group."); return; }
...
function AccountView({ apiBaseUrl, googleAuthAllowed, account, authenticated, sessionAuthenticated, loading, busy, guestProgressAvailable, onAuth, onGoogleAuth, onExitRecovery, onRequestVerification, onConfirmVerification, onImport, onLogout, onReset, badges, places, allPlaces, visits, visitTimestamps, loadPhoto, removePhoto, photoOwnerKey, choosePlace }: AccountViewProps) {
...
function ResetConfirmation({ busy, onCancel, onConfirm }: { busy: boolean; onCancel: () => void; onConfirm: () => Promise<void> }) { const ref = useDialogFocus(onCancel); const [error, setError] = useState(""); return <div ref={ref as React.RefObject<HTMLDivElement>} className="reset-backdrop" role="dialog" aria-modal="true" aria-labelledby="reset-title" onClick={onCancel}><section className="reset-dialog" onClick={(event) => event.stopPropagation()}><div className="reset-icon"><Trash2 size={24} /></div><h2 id="reset-title">Reset all progress?</h2><p>This removes every visited place, earned badge, group, and Wishlist save from your account. Your empty Wishlist will be ready to use again. This cannot be undone.</p>{error && <p className="reset-error" role="alert">{error}</p>}<div><button disabled={busy} onClick={onCancel}>Keep my progress</button><button disabled={busy} onClick={() => { setError(""); void onConfirm().catch((caught) => setError(caught instanceof Error ? caught.message : "Could not reset progress.")); }}>{busy ? "Resetting…" : "Reset everything"}</button></div></section></div>; }
function AccountPostcards({ places, visits, loadPhoto, removePhoto, ownerKey, onOpenPlace }: { places: Place[]; visits: Record<string, Visit>; loadPhoto: (placeId: string) => Promise<Blob>; removePhoto: (placeId: string) => Promise<void>; ownerKey?: string; onOpenPlace: (placeId: string) => void }) {
  const claimed = places.filter((place) => visits[place.id]?.claim);
  return <section className="postcard-journal" aria-label="Visit postcards"><header><div><h3>Visit postcards</h3><p>Private field notes from places you claimed in person.</p></div><span>{claimed.length}</span></header>{claimed.length ? <div className="postcard-grid">{claimed.map((place) => <VisitPostcard key={place.id} place={place} visit={visits[place.id]} loadPhoto={loadPhoto} removePhoto={removePhoto} ownerKey={ownerKey} onOpenPlace={() => onOpenPlace(place.id)} />)}</div> : <p className="postcard-empty">Your first boundary claim will become a postcard here.</p>}</section>;
}
