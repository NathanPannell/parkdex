...
export function ParkdexApp({ apiBaseUrl }: { apiBaseUrl: string }) {
  ...
  return <main className="app-shell"><section className={`map-stage view-${view} ${groupMapMode ? "group-map-mode" : ""}`} aria-label="Parkdex explorer">
    <ParkMap places={selectedGroup ? selectedGroup.places : mapMode === "explored" ? places : mapFiltered} visited={visited} selectedId={selectedId} selectedIds={groupSelectedIds} resetViewRequest={resetViewRequest} showResetControl={view === "map"} onSelect={choosePlace} onBoundaryLoadState={setBoundaryLoadState} mode={selectedGroup ? "discover" : mapMode} currentLocation={location} />
