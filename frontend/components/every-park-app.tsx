...
import { authorityForPlace, groupByRegion, type VisitFilter } from "@/lib/collection";
...
import { MANUAL_CLAIM_PLACE_ID, manualClaimLocation, manualClaimPhoto } from "@/lib/manual-claim";
...
import { PlaceVisualsModal } from "@/components/place-visuals-modal";
import { DEFAULT_PLACE_VISUALS_BASE_URL, loadPlaceVisualIndex, type PlaceVisualEntry } from "@/lib/place-visuals";
...
export function ParkdexApp(props: { apiBaseUrl: string; googleAuthAllowed?: boolean; geolocationAllowed?: boolean; automaticLocationAllowed?: boolean; manualClaimEnabled?: boolean }) {
  const { model, actions } = useParkdexApplication(props);
  const {
    apiBaseUrl, googleAuthAllowed, journal, navigation, selectedId, detailExpanded, mapSearch, mapCategories, collectionSearch,
    collectionCategories, collectionAuthorities, collectionVisitFilter, view, mapSearchDraft, navigationNotice,
    showOnboarding, recoveryActive, location, locationStatus, preciseLocationRequired, preciseLocationBusy, preciseLocationMessage,
    showFilters, searchExpanded, celebrationBadges, viewRevision, resetViewRequest, groupsState, selectedGroup,
    groupSelectedIds, boundaryLoadState, groupMapMode, mapFiltered, mapTotal, mapScope, mapSearchMatches, mapSearchTotal, groups, badgeList,
    earnedBadges, selected, selectedPlacePhotoUrl, selectedPlacePhotoUrls, selectedPlaceImages, claimsAvailable, legacyVisitCreationAvailable, claimFlow, claimFlowResetSignal,
    displayedRecommendation, liveClaimPlace, arrivalPhotoUrl, arrivalImage, approximateClaimLocation, visits, photoOwnerKey,
    resetCleanupPending, resetCleanupBusy, resetPreparationPending, resetCancellationAllowed, mapPlaces, recentPostcard, mapPresentation, places, visited,
    collectionTotal, collectionScope, collectionHasMore, visitedPlaces, visitedTotal, visitedScope, visitedHasMore,
    catalogueTotal, categoryTotals, visitedCategoryTotals, offlineMode,
    visitTimestamps, account, authenticated, loading, persistentSyncMessage, storageUnavailable, groupsErrorStatus,
    guestProgressAvailable, transitionBusy, geolocationAllowed, pendingClaims, offlineClaimRecoveryCount,
    offlineClaimRecoveryMessage, claimRetryBusy, rejectedClaimCount, rejectedClaimsBusy,
  } = model;
  const {
    recommendClaim, createClaim, reconcileClaim, uploadVisitPhoto, loadVisitPhoto, removeVisitPhoto,
  } = journal;
  const {
    updateNavigation, setMapSearch, setMapCategories, setCollectionSearch, setCollectionCategories,
    setCollectionVisitFilter, setMapSearchDraft, setNavigationNotice, setShowOnboarding, setRecoveryActive,
    setShowFilters, setSearchExpanded, setCelebrationBadges, setClaimFlow, setRecentImpression, setBoundaryLoadState,
    completeOnboarding, requestLocation, enablePreciseLocation, resetMapFilters,
    resetCollectionFilters, clearCollectionActiveFilters, toggleSet, rememberGroupPanelScroll, choosePlace,
    openGroupMember, navigate, changeSettingsRoute, applyMapSearch, openMapSearch, switchGuide, closePlace,
    openCollection, viewSelectedGroupOnMap, logoutAndClearGroupHistory, resetProgress, retryResetCleanup, cancelResetPreparation,
    toggleSelected, celebrateClaim, rememberImpression, dismissArrival, clearLiveClaim, retrySync, retryPendingClaims, discardRejectedClaims, completeAuth,
    offlineRetry, searchPlaces, onCollectionLoadMore, onVisitedLoadMore,
    authenticateWithGoogle, requestEmailVerification, confirmEmailVerification, importGuest, onDeleteAccount,
  } = actions;
  ...
  async function startManualClaim() {
    ...
    if (visited.has(MANUAL_CLAIM_PLACE_ID)) {
      setManualClaimError("This park is already in your collection.");
      return;
    }
    setManualClaimStarting(true);
    setManualClaimError("");
    try {
      const recommendation = await recommendClaim({ location: manualClaimLocation() });
      if (recommendation.status !== "recommended" || recommendation.candidate.placeId !== MANUAL_CLAIM_PLACE_ID) {
      ...
      setManualClaimError(error instanceof Error ? error.message : "The test claim could not start. Try again.");
    } finally {
      setManualClaimStarting(false);
    }
  }
  ...
  }, [persistentSyncMessage, storageUnavailable, offlineMode]);
  ...
  return <main className="app-shell field-guide-shell"><div className="skip-controls"><button onClick={focusNavigation}>Skip to navigation</button><button onClick={focusContent}>Skip to content</button></div><section ref={mapStageRef} className={`map-stage view-${view} ${groupMapMode ? "group-map-mode" : ""} ${selected ? "has-selected-place" : ""} ${searchExpanded ? "search-panel-open" : ""}`} aria-label="Parkdex explorer">
    <div id={view === "map" ? "primary-content" : undefined} className="map-content-target" tabIndex={-1} aria-label="Map" /><ParkMap places={mapPlaces} cameraPlace={selected} groupCameraPlaces={selectedGroup?.places} visited={visited} selectedId={selectedId} selectedIds={groupSelectedIds} resetViewRequest={resetViewRequest} showZoomControls={view === "map"} onSelect={choosePlace} onBoundaryLoadState={setBoundaryLoadState} onViewportChange={actions.setMapViewport} mode="explored" presentation={mapPresentation} currentLocation={location} recentPostcard={view === "map" ? recentPostcard : undefined} onOpenPostcard={() => navigate("account")} />
    ...
    {view === "map" && !groupMapMode && <><div className={`map-utility ${searchExpanded ? "search-open" : ""}`} role="toolbar" aria-label="Map utilities">{geolocationAllowed && <button className="locate-button" onClick={requestLocation} aria-label="Show my current location"><LocateFixed size={19} className={locationStatus === "locating" ? "spin" : ""} /></button>}<div className={`search-dock ${searchExpanded ? "expanded" : "collapsed"}`}>
      ...
      {!searchExpanded && (mapSearch.trim() || mapCategories.size > 0 || collectionAuthorities.size > 0) && <div className="applied-map-search"><div role="status"><strong>{mapTotal ? `${mapTotal} ${mapTotal === 1 ? "place" : "places"} found${mapScope === "cached" ? " in saved places" : ""}` : mapScope === "cached" ? "No saved places match" : "No places match"}</strong><span>{[mapSearch.trim() ? `“${mapSearch.trim()}”` : "", ...[...mapCategories].map((category) => categoryLabels[category]), ...[...collectionAuthorities].map(collectionTitle)].filter(Boolean).join(" · ")}</span></div><button onClick={resetMapFilters} aria-label="Clear map search and filters"><X size={17} /><span>Clear</span></button></div>}
      <div className="map-visit-filter segmented" role="group" aria-label="Visit status">{(["all", "visited", "unseen"] as VisitFilter[]).map((value) => <button key={value} aria-pressed={collectionVisitFilter === value} className={collectionVisitFilter === value ? "active" : ""} onClick={() => setCollectionVisitFilter(value)}>{value === "all" ? "All" : value === "visited" ? "Visited" : "Unvisited"}</button>)}</div>
      {searchExpanded && mapSearchDraft.trim() && !showFilters && <div className="search-results" aria-live="polite">{mapSearchMatches.length ? <><p>{mapSearchTotal} {mapSearchTotal === 1 ? "place" : "places"} found{offlineMode ? " in saved places" : ""}{mapSearchTotal > mapSearchMatches.length ? ` · first ${mapSearchMatches.length} shown` : ""}</p>{mapSearchMatches.map((place) => <button key={place.id} className={`category-${place.category}`} onClick={() => { setMapSearch(mapSearchDraft); choosePlace(place.id); setSearchExpanded(false); }}><span><strong>{place.name}</strong><small><i />{categoryLabels[place.category]} · {place.region}</small></span><ArrowUpRight size={17} /></button>)}</> : <p className="empty-search">No {offlineMode ? "saved " : ""}places match “{mapSearchDraft.trim()}”.</p>}</div>}
    </>}
    {view === "map" && !selected && searchExpanded && !mapSearchDraft.trim() && <MapBrowserPanel places={groupMapMode ? mapPlaces : mapFiltered} total={groupMapMode ? mapPlaces.length : mapTotal} scope={groupMapMode ? "full" : mapScope} visited={visited} title={selectedGroup?.name ?? "Explore places"} inert={showFilters} onSelect={choosePlace} />}
    {view !== "map" && <section key={`${view}-${viewRevision}`} onScroll={rememberGroupPanelScroll} className={`feature-panel feature-${view}`}><button className="content-skip" onClick={focusNavigation}>Skip to navigation</button><div id="primary-content" tabIndex={-1} className="primary-content-target" aria-label={view === "collection" ? "Field Guide" : view === "badges" ? "Badges" : view === "groups" ? "Collections" : "My Dex"} />{view === "collection" && <CollectionView groups={groups} total={collectionTotal} scope={collectionScope} hasMore={collectionHasMore} catalogueTotal={catalogueTotal} categoryTotals={categoryTotals} visitedCategoryTotals={visitedCategoryTotals} visited={visited} search={collectionSearch} setSearch={setCollectionSearch} selectedCategories={collectionCategories} authorities={collectionAuthorities} visitFilter={collectionVisitFilter} setVisitFilter={setCollectionVisitFilter} toggleCategory={(value) => toggleSet(setCollectionCategories, value)} resetFilters={resetCollectionFilters} clearActiveFilters={clearCollectionActiveFilters} choosePlace={choosePlace} onLoadMore={onCollectionLoadMore} />}{view === "groups" && authenticated && !recoveryActive && <GroupsView searchPlaces={searchPlaces} groups={groupsState.groups} selectedGroupId={groupsState.selectedGroupId} loading={groupsState.loading} retrying={groupsState.retrying} error={groupsErrorStatus} busy={groupsState.busy} offline={groupsState.offline} syncStatus={groupsState.syncStatus} syncMessage={groupsState.syncMessage} pendingMemberships={groupsState.pendingMemberships} onRetry={groupsState.retry} onSelect={groupsState.selectGroup} onClear={() => groupsState.selectGroup(null)} onCreate={groupsState.create} onRename={groupsState.rename} onDelete={groupsState.remove} onAddPlace={groupsState.addPlace} onRemovePlace={groupsState.removePlace} onViewMap={viewSelectedGroupOnMap} onOpenPlace={openGroupMember} />}{view === "badges" && <><button className="groups-back" onClick={() => navigate("account")}><ArrowLeft size={18} />My Dex</button><BadgesView badges={badgeList} earned={earnedBadges} visited={visited} onOpenPlace={choosePlace} /></>}{(view === "account" || (view === "groups" && (!authenticated || recoveryActive))) && <AccountView section={view === "groups" ? "collections" : "account"} settingsRoute={navigation.settingsOpen} onSettingsRouteChange={changeSettingsRoute} apiBaseUrl={apiBaseUrl} googleAuthAllowed={googleAuthAllowed} account={account} authenticated={authenticated && !recoveryActive} sessionAuthenticated={authenticated} loading={loading} busy={transitionBusy || Boolean(claimFlow) || resetCleanupBusy} guestProgressAvailable={guestProgressAvailable} onAuth={completeAuth} onGoogleAuth={authenticateWithGoogle} onExitRecovery={() => setRecoveryActive(false)} onRequestVerification={requestEmailVerification} onConfirmVerification={confirmEmailVerification} onImport={importGuest} onLogout={logoutAndClearGroupHistory} onReset={resetProgress} onCancelResetPreparation={cancelResetPreparation} resetCancellationAllowed={resetCancellationAllowed} onDeleteAccount={onDeleteAccount} badges={badgeList} places={visitedPlaces} visitedTotal={visitedTotal} visitedScope={visitedScope} visitedHasMore={visitedHasMore} onVisitedLoadMore={onVisitedLoadMore} visited={visited} visits={visits} visitTimestamps={visitTimestamps} loadPhoto={loadVisitPhoto} removePhoto={removeVisitPhoto} photoOwnerKey={photoOwnerKey} choosePlace={choosePlace} onBrowseBadges={() => navigate("badges")} onShowGuide={() => setShowOnboarding(true)} manualClaimEnabled={props.manualClaimEnabled} manualClaimStarting={manualClaimStarting} manualClaimClaimed={visited.has(MANUAL_CLAIM_PLACE_ID)} manualClaimError={manualClaimError} onManualClaim={() => void startManualClaim()} />}</section>}
    ...
    {resetPreparationPending && <p className="navigation-notice" role="status">A progress reset needs to be finished before collection changes can resume.{resetCancellationAllowed ? <button className="claim-refresh" disabled={resetCleanupBusy} onClick={() => void cancelResetPreparation().catch((error) => notifyError(error, "Saved collections remain paused until they can be refreshed."))}>{resetCleanupBusy ? "Refreshing…" : "Refresh and resume collections"}</button> : <button className="claim-refresh" onClick={() => navigate("account")}>Open account to finish reset</button>}</p>}
    {resetCleanupPending && <p className="navigation-notice" role="status">Saved collection cleanup is still pending.<button className="claim-refresh" disabled={resetCleanupBusy} onClick={() => void retryResetCleanup()}>{resetCleanupBusy ? "Retrying…" : "Retry collection cleanup"}</button></p>}
    {(offlineMode || persistentSyncMessage || storageUnavailable || pendingClaims > 0 || offlineClaimRecoveryCount > 0 || rejectedClaimCount > 0) && <div ref={connectionStatusRef} className="connection-status" role="status" aria-live="polite" aria-atomic="false">{offlineMode && <p className="sync-note offline-mode-note"><strong>You are in offline mode.</strong> Showing saved places only.<button onClick={() => void offlineRetry()}>Retry connection</button></p>}{persistentSyncMessage && <p className="sync-note">{persistentSyncMessage}{persistentSyncMessage.includes("waiting") && <button onClick={() => void retrySync()}>Retry</button>}</p>}{(pendingClaims > 0 || offlineClaimRecoveryCount > 0) && <p className="sync-note claim-sync-note">{pendingClaims > 0 ? `${pendingClaims} saved ${pendingClaims === 1 ? "visit is" : "visits are"} waiting to sync on this device.` : `${offlineClaimRecoveryCount} saved ${offlineClaimRecoveryCount === 1 ? "visit needs" : "visits need"} attention before syncing.`}{offlineClaimRecoveryMessage && <span>{offlineClaimRecoveryMessage}</span>}<button disabled={claimRetryBusy} onClick={() => void retryPendingClaims()}>{claimRetryBusy ? "Retrying…" : "Retry visit sync"}</button></p>}{rejectedClaimCount > 0 && <p className="sync-note rejected-claim-note">{rejectedClaimCount} saved {rejectedClaimCount === 1 ? "visit was" : "visits were"} rejected and need attention.<button disabled={rejectedClaimsBusy} onClick={() => void discardRejectedClaims()}>{rejectedClaimsBusy ? "Dismissing…" : "Dismiss rejected visits"}</button></p>}{storageUnavailable && <p className="storage-note">Private storage is blocked; guest progress lasts for this tab.</p>}</div>}
    {selected && <PlaceDetail key={selected.id} place={selected} visit={visits[selected.id]} visited={visited.has(selected.id)} busy={transitionBusy} authenticated={authenticated} claimsAvailable={claimsAvailable} legacyVisitCreationAvailable={legacyVisitCreationAvailable} recommendClaim={recommendClaim} createClaim={createClaim} reconcileClaim={reconcileClaim} uploadPhoto={uploadVisitPhoto} loadPhoto={loadVisitPhoto} removePhoto={removeVisitPhoto} ownerKey={photoOwnerKey} photoUrl={selectedPlacePhotoUrl} photoUrls={selectedPlacePhotoUrls} images={selectedPlaceImages} onClaimed={celebrateClaim} onOpenPlace={choosePlace} groupsState={groupsState} boundaryState={boundaryLoadState} expanded={detailExpanded} onExpand={() => updateNavigation({ detailExpanded: !detailExpanded })} onClose={closePlace} onToggle={() => toggleSelected(selected)} openCollection={openCollection} />}
...
function MapBrowserPanel({ places, total, scope, visited, title, inert, onSelect }: { places: Place[]; total: number; scope: "full" | "cached"; visited: ReadonlySet<string>; title: string; inert: boolean; onSelect: (id: string) => void }) {
  const regions = groupByRegion(places);
  return <aside className="map-browser-panel" aria-label="Places on the map" inert={inert}>
    <header className="map-browser-heading"><div><h2>{title}</h2><p>{scope === "cached" ? "Saved places: " : ""}{places.length} shown{total > places.length ? ` of ${total}` : ""} on the map</p></div></header>
...
function PlaceDetail({ expanded, onExpand, place, visit, visited, busy, authenticated, claimsAvailable, legacyVisitCreationAvailable, recommendClaim, createClaim, reconcileClaim, uploadPhoto, loadPhoto, removePhoto, ownerKey, photoUrl, photoUrls, images: savedImages, onClaimed, onOpenPlace, groupsState, boundaryState, onClose, onToggle, openCollection }: {
  expanded: boolean;
  onExpand: () => void;
  place: Place;
  visit?: Visit;
  ...
  const detailRef = useDialogFocus(visualsOpen ? undefined : onClose, modal && !visualsOpen);
  const visualsBaseUrl = process.env.NEXT_PUBLIC_PARK_VISUALS_BASE_URL || DEFAULT_PLACE_VISUALS_BASE_URL;
  const [visualLookup, setVisualLookup] = useState<{ placeId: string; entry: PlaceVisualEntry | null; error: boolean } | null>(null);
  const visualEntry = visualLookup?.placeId === place.id ? visualLookup.entry : null;
  const visualsError = visualLookup?.placeId === place.id && visualLookup.error;
  useEffect(() => {
    let active = true;
    void loadPlaceVisualIndex(visualsBaseUrl).then((index) => {
      if (active) setVisualLookup({ placeId: place.id, entry: index.get(place.id) ?? null, error: false });
    }).catch(() => {
      if (active) setVisualLookup({ placeId: place.id, entry: null, error: true });
    });
    return () => { active = false; };
  }, [place.id, visualsBaseUrl]);
  const images = savedImages ?? getPlaceImages(place.id);
  ...
  return <article ref={detailRef as React.RefObject<HTMLElement>} role="dialog" aria-modal={modal && !visualsOpen ? "true" : undefined} className={`place-sheet ${expanded ? "place-sheet-full" : ""} ${hasImage ? "with-photo" : "without-photo"} ${authenticated ? "signed-in" : "guest place-sheet-guest"}`} data-authenticated={authenticated ? "true" : "false"} aria-labelledby="place-detail-title">
    ...
    <div className="place-sheet-content">
      ...
      {visualEntry && <button className="place-visuals-launch" type="button" onClick={() => setVisualsOpen(true)}><Layers size={18} />Map views</button>}
      {visualsError && <p className="place-visuals-index-error" role="status">Map views are temporarily unavailable.</p>}
    ...
    {visualsOpen && visualEntry && <PlaceVisualsModal placeName={place.name} entry={visualEntry} baseUrl={visualsBaseUrl} onClose={() => setVisualsOpen(false)} />}
  </article>;
}
...
function CategoryProgress({ total, categoryTotals, visitedCategoryTotals }: { total: number; categoryTotals: Record<PlaceCategory, number>; visitedCategoryTotals: Record<PlaceCategory, number> }) {
  const counts = categories.map((category) => ({ category, total: categoryTotals[category] ?? 0, visited: visitedCategoryTotals[category] ?? 0 }));
  const collected = counts.reduce((sum, count) => sum + count.visited, 0);
  return <section className="collection-progress" aria-label="Visit progress"><header><strong>{collected} of {total} visited</strong></header><div className="collection-progress-track" role="progressbar" aria-label={`${collected} of ${total} places visited`} aria-valuemin={0} aria-valuemax={total} aria-valuenow={collected}><div>{counts.map(({ category, visited: categoryVisited }) => <span key={category} className={`category-${category}`} style={{ width: `${total ? categoryVisited / total * 100 : 0}%` }} />)}</div></div><ul>{counts.map(({ category, total: categoryTotal, visited: categoryVisited }) => <li key={category} className={`category-${category}`}><i /><span>{categoryLabels[category]}</span><b>{categoryVisited}/{categoryTotal}</b></li>)}</ul></section>;
}
...
function CollectionView({ groups, total, scope, hasMore, catalogueTotal, categoryTotals, visitedCategoryTotals, visited, search, setSearch, selectedCategories, authorities, visitFilter, setVisitFilter, toggleCategory, resetFilters, clearActiveFilters, choosePlace, onLoadMore }: { groups: ReturnType<typeof groupByRegion>; total: number; scope: "full" | "cached"; hasMore: boolean; catalogueTotal: number; categoryTotals: Record<PlaceCategory, number>; visitedCategoryTotals: Record<PlaceCategory, number>; visited: Set<string>; search: string; setSearch: (v: string) => void; selectedCategories: Set<PlaceCategory>; authorities: Set<string>; visitFilter: VisitFilter; setVisitFilter: (v: VisitFilter) => void; toggleCategory: (v: PlaceCategory) => void; resetFilters: () => void; clearActiveFilters: () => void; choosePlace: (id: string) => void; onLoadMore: () => void }) {
...
function GroupsView({ searchPlaces, groups, selectedGroupId, loading, retrying, error, busy, offline, syncStatus, syncMessage, pendingMemberships, onRetry, onSelect, onClear, onCreate, onRename, onDelete, onAddPlace, onRemovePlace, onViewMap, onOpenPlace }: GroupsViewProps) {
  const [name, setName] = useState("");
...
function GroupDetail({ group, searchPlaces, busy, offline, editingId, editingName, actionError, setEditingId, setEditingName, onRename, onDelete, onAddPlace, onRemovePlace, onViewMap, onOpenPlace }: { group: import("@/lib/groups").Group; searchPlaces: (query: string) => Promise<Place[]>; busy: boolean; offline: boolean; editingId: string | null; editingName: string; actionError: string; setEditingId: (value: string | null) => void; setEditingName: (value: string) => void; onRename: (event: React.FormEvent, id: string) => Promise<void>; onDelete: () => void; onAddPlace: (groupId: string, placeId: string) => Promise<void>; onRemovePlace: (groupId: string, placeId: string) => Promise<void>; onViewMap: () => void; onOpenPlace: (id: string) => void }) {
  const [addSearch, setAddSearch] = useState("");
  const [searchResult, setSearchResult] = useState<{ query: string; places: Place[]; error: string }>({ query: "", places: [], error: "" });
  const searchPlacesRef = useRef(searchPlaces);
  useEffect(() => { searchPlacesRef.current = searchPlaces; }, [searchPlaces]);
  useEffect(() => {
    const query = addSearch.trim();
    if (!query) return;
    let active = true;
    const timer = window.setTimeout(() => {
      void searchPlacesRef.current(query).then((results) => {
        if (active) setSearchResult({ query, places: results, error: "" });
      }).catch((error: unknown) => {
        if (active) setSearchResult({ query, places: [], error: error instanceof Error ? error.message : "Could not search places." });
      });
    }, 180);
...
type AccountViewProps = {
  ...
  onImport: () => Promise<void>; onLogout: () => Promise<void>; onReset: () => Promise<void>; onCancelResetPreparation: () => Promise<void>; resetCancellationAllowed: boolean; onDeleteAccount?: () => Promise<AccountDeletionResult>;
  badges: Achievement[]; places: Place[]; visitedTotal: number; visitedScope: "full" | "cached"; visitedHasMore: boolean; onVisitedLoadMore: () => void; visited: Set<string>; visits: Record<string, Visit>; visitTimestamps: Record<string, string>; loadPhoto?: (placeId: string) => Promise<Blob>; removePhoto?: (placeId: string) => Promise<void>; photoOwnerKey?: string; choosePlace: (id: string) => void;
  manualClaimEnabled?: boolean; manualClaimStarting?: boolean; manualClaimClaimed?: boolean; manualClaimError?: string; onManualClaim?: () => void;
};
...
function AccountView({ section, settingsRoute, onSettingsRouteChange, onShowGuide, onBrowseBadges, apiBaseUrl, googleAuthAllowed, account, authenticated, sessionAuthenticated, loading, busy, guestProgressAvailable, onAuth, onGoogleAuth, onExitRecovery, onRequestVerification, onConfirmVerification, onImport, onLogout, onReset, onCancelResetPreparation, resetCancellationAllowed, onDeleteAccount, badges, places, visitedTotal, visitedScope, visitedHasMore, onVisitedLoadMore, visited, visits, visitTimestamps, loadPhoto, removePhoto, photoOwnerKey, choosePlace, manualClaimEnabled, manualClaimStarting, manualClaimClaimed, manualClaimError, onManualClaim }: AccountViewProps) {
  ...
  const accountController = useAccountController({ apiBaseUrl, onGoogleAuth, onConfirmVerification, onRequestVerification, setFormBusy, setError, setNotice });
  const { resetToken, authConfig } = accountController;
  const deletionIntent = typeof window !== "undefined" && new URLSearchParams(window.location.search).get("action") === "delete-account";
  const [mode, setMode] = useState<"login" | "register" | "forgot" | "sent" | "reset" | "expired">(() => resetToken ? "reset" : deletionIntent ? "login" : "register");
  ...
  const deletionIntentHandled = useRef(false), earned = badges.filter((badge) => badge.earned);
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      if (mode === "forgot") { await accountController.requestPasswordReset(email); setMode("sent"); }
      else if (mode === "reset") { if (password !== confirmPassword) throw new Error("Passwords do not match."); await accountController.confirmPasswordReset(resetToken, password); if (sessionAuthenticated) await onLogout(); setMode("login"); setPassword(""); setConfirmPassword(""); setNotice("Password reset. Log in with your new password."); }
      else if (mode === "login" || mode === "register") await onAuth(mode, email, password);
    } catch (caught) {
      if (mode === "reset" && authStatus(caught) === 400) {
        setMode("expired"); setError("");
        notifyError(caught, "That password reset link is no longer valid. Request a fresh link to continue.");
      } else if (mode === "forgot") {
        const message = authStatus(caught) === 429 ? "Too many requests. Try again in a few minutes." : "We couldn’t send the reset email. Please try again.";
        setError(message);
        notifyError(caught, message);
      } else {
        const validationMismatch = mode === "reset" && caught instanceof Error && caught.message === "Passwords do not match.";
        setError(caught instanceof Error ? caught.message : "Could not continue.");
        if (!validationMismatch) notifyError(caught, "Could not continue with this account action.");
      }
  ...
  if (authenticated) return <>
    ...
    {settingsOpen ? <section className="dex-account-settings"><button className="secondary-action" onClick={onShowGuide}><BookOpen size={18} />How Parkdex works</button>{manualClaimEnabled && <div className="manual-claim-setting"><h3>Test a park claim</h3><p>Open Englishman River Regional Park on the map, use a simulated location and generated photo, then save a real visit to this account.</p><button type="button" className="secondary-action" disabled={busy || manualClaimStarting || manualClaimClaimed} onClick={onManualClaim}><MapPin size={18} />{manualClaimStarting ? "Opening park…" : manualClaimClaimed ? "Englishman River already claimed" : "Try Englishman River claim"}</button>{manualClaimError && <p className="auth-error" role="alert">{manualClaimError}</p>}</div>}<h3>Account</h3><p>{account?.email ?? "Account details will refresh online"}</p>{notice && <p className="auth-notice" role="status">{notice}</p>}{error && <p className="auth-error" role="alert">{error}</p>}{account && !account.emailVerified && <section className="verification-card"><MailCheck size={22} /><div><strong>Verify your email</strong><p>Confirm that this email address belongs to you.</p></div><button disabled={formBusy || !authConfig?.emailEnabled} onClick={() => void accountController.resendVerification()}>{authConfig?.emailEnabled ? "Send verification" : "Email unavailable"}</button></section>}{guestProgressAvailable && <div className="import-card"><strong>Guest progress found on this device</strong><p>Add it to this account? This optional step keeps shared-device collections separate.</p><button disabled={busy} onClick={() => void onImport()}>{busy ? "Working…" : "Add guest progress"}</button></div>}<button disabled={busy} className="secondary-action" onClick={openPasswordReset}><MailCheck size={18} />Reset password by email</button><button disabled={busy} className="secondary-action" onClick={() => void onLogout()}><LogOut size={18} />{busy ? "Signing out…" : "Sign out"}</button><button disabled={busy} className="reset-action" onClick={() => setConfirmReset(true)}><Trash2 size={18} />Reset my progress</button>{onDeleteAccount && <div className="account-delete-section"><h3>Delete account</h3><p>This permanently removes your account and saved progress.</p><button disabled={busy} className="reset-action" onClick={openAccountDeletion}><Trash2 size={18} />Delete account</button></div>}<ReleaseFooter />{deletionDialog}</section> : <>
      <dl className="dex-summary"><div><dt>Places visited</dt><dd>{visited.size}</dd></div><div><dt>Saved visits</dt><dd>{Object.keys(visits).length}</dd></div><div><dt>Postcards</dt><dd>{Object.values(visits).filter((visit) => visit.claim).length}</dd></div></dl>
      {loadPhoto && removePhoto && <PostcardCollection places={places} visits={visits} loadPhoto={loadPhoto} removePhoto={removePhoto} ownerKey={photoOwnerKey} onOpenPlace={choosePlace} />}{visitedHasMore && <button className="collection-load-more" onClick={onVisitedLoadMore}>Load more saved places and postcards</button>}<AccountShelf title="Badges" count={earned.length} items={earned.slice(0, 4).map((badge) => ({ id: badge.id, name: badge.name, kind: "badge" as const, image: imageMap[badge.species]?.src, date: badge.earnedAt }))} onSeeAll={() => setExpanded("badges")} onSelect={selectShelfItem} /><AccountShelf title={visitedScope === "cached" ? "Saved places" : "Places"} count={visitedTotal} items={places.slice(0, 4).map((place) => ({ id: place.id, name: place.name, kind: "place" as const, image: getPlaceImage(place.id)?.thumbnail.src, date: visitTimestamps[place.id] }))} onSeeAll={() => setExpanded("places")} onSelect={selectShelfItem} /><button className="dex-badge-guide" onClick={onBrowseBadges}><Award size={20} />Explore all badges<ChevronRight size={18} /></button>
    </>}{expanded && <CollectionModal title={expanded === "badges" ? "All badges" : visitedScope === "cached" ? "Saved places" : "All places"} items={expanded === "badges" ? earned.map((badge) => ({ id: badge.id, name: badge.name, kind: "badge" as const, image: imageMap[badge.species]?.src, date: badge.earnedAt })) : places.map((place) => ({ id: place.id, name: place.name, kind: "place" as const, image: getPlaceImage(place.id)?.thumbnail.src, date: visitTimestamps[place.id] }))} hasMore={expanded === "places" && visitedHasMore} onLoadMore={onVisitedLoadMore} onClose={() => setExpanded(null)} onSelect={selectShelfItem} />}{selectedBadge && <BadgeDetail badge={selectedBadge} visited={visited} onOpenPlace={choosePlace} onClose={() => setSelectedBadge(null)} />}{confirmReset && <ResetConfirmation busy={busy} onCancel={() => { if (!resetCancellationAllowed) { setConfirmReset(false); return; } void onCancelResetPreparation().then(() => setConfirmReset(false)).catch((caught) => notifyError(caught, "Saved collections remain paused until they can be refreshed.")); }} onConfirm={async () => { try { await onReset(); setConfirmReset(false); } catch { /* The application controller reports failures through its toast. */ } }} />}</>;
...
function ResetConfirmation({ busy, onCancel, onConfirm }: { busy: boolean; onCancel: () => void; onConfirm: () => Promise<void> }) { const ref = useDialogFocus(onCancel); return <div ref={ref as React.RefObject<HTMLDivElement>} className="reset-backdrop" role="dialog" aria-modal="true" aria-labelledby="reset-title" onClick={onCancel}><section className="reset-dialog" onClick={(event) => event.stopPropagation()}><div className="reset-icon"><Trash2 size={24} /></div><h2 id="reset-title">Reset all progress?</h2><p>This removes all visited places, earned badges, collections, Wishlist saves, and private visit photos from your account. Your empty Wishlist will be ready to use again. This cannot be undone.</p><div><button disabled={busy} onClick={onCancel}>Keep my progress</button><button disabled={busy} onClick={() => { void onConfirm().catch(() => undefined); }}>{busy ? "Resetting…" : "Reset everything"}</button></div></section></div>; }
