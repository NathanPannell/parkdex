...
export function ParkdexApp({ apiBaseUrl, googleAuthAllowed = true, geolocationAllowed = true }: { apiBaseUrl: string; googleAuthAllowed?: boolean; geolocationAllowed?: boolean }) {
  ...
  return <main className="app-shell"><div className="skip-controls"><button onClick={focusNavigation}>Skip to navigation</button><button onClick={focusContent}>Skip to content</button></div><section className={`map-stage view-${view} ${groupMapMode ? "group-map-mode" : ""}`} aria-label="Parkdex explorer">
    ...
    {selected && <PlaceDetail key={selected.id} place={selected} visited={visited.has(selected.id)} busy={transitionBusy} authenticated={authenticated} groupsState={groupsState} boundaryState={boundaryLoadState} onClose={() => { updateNavigation({ selectedId: null }, "push"); setSearchExpanded(false); }} onToggle={() => toggleSelected(selected)} openCollection={openCollection} />}
...
function PlaceDetail({ place, visited, busy, authenticated, groupsState, boundaryState, onClose, onToggle, openCollection }: { place: Place; visited: boolean; busy: boolean; authenticated: boolean; groupsState: ReturnType<typeof useGroups>; boundaryState: BoundaryLoadState; onClose: () => void; onToggle: () => void; openCollection: (category?: PlaceCategory, authority?: string) => void }) {
...
function GroupsView({ places, groups, selectedGroupId, loading, error, busy, onRetry, onSelect, onClear, onCreate, onRename, onDelete, onAddPlace, onRemovePlace, onViewMap, onOpenPlace }: GroupsViewProps) {
  const [name, setName] = useState("");
...
function AccountView({ apiBaseUrl, googleAuthAllowed, account, authenticated, sessionAuthenticated, loading, busy, guestProgressAvailable, onAuth, onGoogleAuth, onExitRecovery, onRequestVerification, onConfirmVerification, onImport, onLogout, onReset, badges, places, visitTimestamps, choosePlace }: AccountViewProps) {
