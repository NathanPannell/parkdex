...
export function ParkdexApp({ apiBaseUrl }: { apiBaseUrl: string }) {
  ...
  return <main className="app-shell"><section className={`map-stage view-${view} ${groupMapMode ? "group-map-mode" : ""}`} aria-label="Parkdex explorer">
    ...
    {!groupMapMode && <header className="expedition-header"><div className="brand-mark"><Trees size={22} /></div><div className="brand-copy"><h1>Parkdex</h1><p>A completionist map of Vancouver Island</p></div></header>}
    {groupMapMode && <button className="group-map-exit" onClick={() => setView("groups")}><ArrowLeft size={17} />Back to group</button>}
