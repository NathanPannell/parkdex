...
import { ArrowLeft, ArrowUpRight, Award, Check, ChevronDown, ChevronRight, Compass, Heart, LandPlot, ListFilter, ListPlus, LocateFixed, LogIn, LogOut, MailCheck, Map as MapIcon, MapPin, Pencil, Plus, RotateCcw, Search, Trees, Trash2, UserRound, X } from "lucide-react";
...
function GlobalProgress({ value, total, compact = false }: { value: number; total: number; compact?: boolean }) {
  ...
  return <div className={`global-progress ${compact ? "group-map-progress" : ""}`} role="progressbar" aria-label="Parkdex progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={settled.percentage} aria-valuetext={`${settled.percentage.toFixed(1)}% · ${settled.count} of ${settled.total} places visited`}><strong aria-hidden="true">{displayedPercentage.toFixed(1)}%</strong></div>;
}
