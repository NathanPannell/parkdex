...
export function ParkdexApp({ apiBaseUrl }: { apiBaseUrl: string }) {
  ...
  const [groupMapAdding, setGroupMapAdding] = useState(false), [viewRevision, setViewRevision] = useState(0), [resetViewRequest, setResetViewRequest] = useState(0);
  ...
  function navigate(next: View) {
    ...
    if (next === "map") { groupsState.selectGroup(null); if (resetting) { resetMapFilters(); setMapMode("explored"); setSearchExpanded(false); setResetViewRequest((current) => current + 1); } }
  ...
  return <main className="app-shell"><section className={`map-stage view-${view} ${groupMapMode ? "group-map-mode" : ""}`} aria-label="Parkdex explorer">
    <ParkMap places={selectedGroup ? (groupMapAdding ? places : selectedGroup.places) : mapMode === "explored" ? places : mapFiltered} visited={visited} selectedId={selectedId} selectedIds={groupSelectedIds} resetViewRequest={resetViewRequest} onSelect={(id) => { if (groupMapAdding && selectedGroup) { if (!groupSelectedIds.has(id)) void groupsState.addPlace(selectedGroup.id, id); return; } choosePlace(id); }} onBoundaryLoadState={setBoundaryLoadState} mode={selectedGroup ? "discover" : mapMode} currentLocation={location} />
