...
import { ArrowLeft, ArrowUpRight, Award, Check, ChevronDown, Compass, Heart, KeyRound, LandPlot, ListFilter, ListPlus, LocateFixed, LogIn, LogOut, MailCheck, Map as MapIcon, MapPin, Pencil, Plus, RotateCcw, Search, Trees, Trash2, UserRound, X } from "lucide-react";
...
export function ParkdexApp({ apiBaseUrl }: { apiBaseUrl: string }) {
  ...
  function navigate(next: View) { setSelectedId(null); setShowNearby(false); setShowFilters(false); if (next === "map" || (next === "groups" && view === "groups")) groupsState.selectGroup(null); setView((current) => ((next === "collection" || next === "groups" || next === "badges") && current === next ? "map" : next)); }
  ...
  return <main className="app-shell"><section className={`map-stage view-${view} ${groupMapMode ? "group-map-mode" : ""}`} aria-label="Parkdex explorer">
    <ParkMap places={selectedGroup ? selectedGroup.places : mapMode === "explored" ? places : mapFiltered} visited={visited} selectedId={selectedId} selectedIds={groupSelectedIds} onSelect={choosePlace} onBoundaryLoadState={setBoundaryLoadState} mode={selectedGroup ? "discover" : mapMode} currentLocation={location} />
    {!groupMapMode && <header className="expedition-header"><div className="brand-mark"><Trees size={22} /></div><div className="brand-copy"><h1>Parkdex</h1><p>A completionist map of Vancouver Island</p></div><nav className="desktop-top-nav" aria-label="Primary navigation"><Nav active={view === "map"} click={() => navigate("map")} icon={<MapIcon size={18} />} label="Map" ariaName="Map tab" />{authenticated && <Nav active={view === "collection"} click={() => navigate("collection")} icon={<Trees size={18} />} label="Places" ariaName="Places tab" count={visited.size} />}{authenticated && <Nav active={view === "groups"} click={() => navigate("groups")} icon={<MapPin size={18} />} label="Groups" ariaName="Groups tab" count={groupsState.groups.length} />}{authenticated && <Nav active={view === "badges"} click={() => navigate("badges")} icon={<Award size={18} />} label="Badges" ariaName="Badges tab" count={earnedBadges} />}<Nav active={view === "account"} click={() => navigate("account")} icon={<UserRound size={18} />} label="Account" ariaName="Account tab" /></nav></header>}
    {groupMapMode && <button className="group-map-exit" onClick={() => navigate("groups")}><ArrowLeft size={17} />Back to group</button>}
    {view === "map" && !groupMapMode && <><div className={`map-utility ${searchExpanded || mapSearch ? "search-open" : ""}`} role="toolbar" aria-label="Map utilities"><div className="map-mode-switch"><button className={mapMode === "explored" ? "active" : ""} onClick={() => { setMapMode("explored"); setShowNearby(false); }}><Trees size={16} /><span>My map</span></button><button className={mapMode === "discover" ? "active" : ""} onClick={() => setMapMode("discover")}><Compass size={16} /><span>Find places</span></button><button className="locate-button" onClick={requestLocation} aria-label="Show my current location"><LocateFixed size={17} className={locationStatus === "locating" ? "spin" : ""} /></button></div><div className={`search-dock ${searchExpanded || mapSearch ? "expanded" : "collapsed"}`}>
    ...
    {selected && <article className="place-sheet"><button className="sheet-close" onClick={() => setSelectedId(null)} aria-label="Close place details"><X size={18} /></button><button className={`place-category category-${selected.category}`} onClick={() => openCollection(selected.category)}>{categoryLabels[selected.category]}<ArrowUpRight size={13} /></button><h2>{selected.name}</h2><PlaceImage place={selected} variant="card" /><p className="place-region"><MapPin size={15} />{selected.region}</p><button className="place-collection-link" onClick={() => openCollection(undefined, authorityForPlace(selected))}>{authorityForPlace(selected)}<ArrowUpRight size={13} /></button><PlaceProvenance place={selected} boundaryState={boundaryLoadState} /><p className="place-description">{selected.description}</p><div className="sheet-actions"><button disabled={transitionBusy} className={`visit-button ${visited.has(selected.id) ? "is-visited" : ""}`} onClick={() => toggleSelected(selected)}>{visited.has(selected.id) ? <><RotateCcw size={19} />Visited · undo</> : <><Check size={20} />Mark as visited</>}</button>{authenticated && <GroupActions place={selected} groups={groupsState.groups} busy={groupsState.busy} onCreate={async (name, placeIds) => { const created = await groupsState.create(name, placeIds); groupsState.selectGroup(null); return created; }} onAddPlace={groupsState.addPlace} onRemovePlace={groupsState.removePlace} />}</div></article>}
    <nav className={`thumb-nav ${authenticated ? "authenticated" : "guest"}`}><div className="nav-cluster nav-cluster-map"><span>Map</span><Nav active={view === "map"} click={() => navigate("map")} icon={<MapIcon size={20} />} label="Map" />{authenticated && <Nav active={view === "collection"} click={() => navigate("collection")} icon={<Trees size={20} />} label="Places" count={visited.size} />}</div>{authenticated && <div className="nav-cluster nav-cluster-stuff"><span>My stuff</span><Nav active={view === "groups"} click={() => navigate("groups")} icon={<MapPin size={20} />} label="Groups" count={groupsState.groups.length} /><Nav active={view === "badges"} click={() => navigate("badges")} icon={<Award size={20} />} label="Badges" count={earnedBadges} /><Nav active={view === "account"} click={() => navigate("account")} icon={<UserRound size={20} />} label="Account" /></div>}{!authenticated && <Nav active={view === "account"} click={() => navigate("account")} icon={<UserRound size={20} />} label="Account" />}</nav>
...
function Nav({ active, click, icon, label, count, ariaName }: { active: boolean; click: () => void; icon: React.ReactNode; label: string; count?: number; ariaName?: string }) { const name = ariaName ?? label; return <button className={active ? "active" : ""} onClick={click} aria-label={active && (label === "Places" || label === "Groups" || label === "Badges") ? `Close ${name} and return to map` : name}>{icon}<span>{label}</span>{count !== undefined && <b>{count}</b>}</button>; }
...
function AccountView({ apiBaseUrl, account, authenticated, sessionAuthenticated, loading, busy, guestProgressAvailable, onAuth, onGoogleAuth, onChangePassword, onSetPassword, onRequestVerification, onConfirmVerification, onImport, onLogout, onReset, badges, places, visitTimestamps, choosePlace }: AccountViewProps) {
  ...
  async function submitPasswordChange(event: React.FormEvent) { event.preventDefault(); setError(""); if (newPassword !== confirmPassword) { setError("Passwords do not match."); return; } setFormBusy(true); try { if (account?.hasPassword === false) await onSetPassword(newPassword); else await onChangePassword(currentPassword, newPassword); setNotice(account?.hasPassword === false ? "Password set. Log in again." : "Password changed. Log in again."); } catch (caught) { setError(caught instanceof Error ? caught.message : "Could not update your password."); } finally { setFormBusy(false); } }
