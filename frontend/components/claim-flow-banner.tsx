...
export function ClaimFlowBanner({
  ...
  return <aside ref={dialogRef} className={`impression-flow impression-flow--${flowScreen}`} role="dialog" aria-modal="true" aria-labelledby="impression-flow-title" aria-live="polite">
    {flowScreen === "arrival" && <section className="impression-arrival">
      <div className="impression-arrival-art" aria-hidden={placeImage ? undefined : "true"}>{placeImage ? <img src={placeImage.detail.src} alt={placeImage.alt} /> : <div className="impression-arrival-placeholder"><MapPin size={44} /><span>Place boundary</span></div>}{placeImage && <small className="impression-arrival-credit">Photo <a href={placeImage.sourceUrl} target="_blank" rel="noreferrer">{placeImage.creator}</a>, <a href={placeImage.licenseUrl} target="_blank" rel="noreferrer">{placeImage.license}</a></small>}</div>
