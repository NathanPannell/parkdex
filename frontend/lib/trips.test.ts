...
import { addTripPlace, createTrip, listTrips, normalizeTrips, removeTripPlace, TripsApiError } from "./trips";
...
describe("groups API client", () => {
  it("normalizes group place ids and keeps wishlist metadata", () => {
    const [group, wishlist] = normalizeTrips([{ id: "g1", name: "Weekend", placeIds: [place.id], places: [place] }, { id: "w1", name: "Wishlist", isWishlist: true, placeIds: [] }]);
  ...
  it("uses the authenticated groups contract and deduplicates create members", async () => {
    ...
    await createTrip(request, "Weekend", [place.id, place.id]);
  ...
  it("adds and removes membership with bounded placeIds payloads", async () => {
    const request = vi.fn<(path: string, init?: RequestInit) => Promise<Response>>(() => response({ id: "g1", name: "Weekend", placeIds: [place.id], places: [place] }));
    await addTripPlace(request, "g1", place.id);
    await removeTripPlace(request, "g1", place.id);
  ...
  it("surfaces API status and detail without leaking response internals", async () => {
    ...
    await expect(listTrips(request)).rejects.toMatchObject({ name: "TripsApiError", status: 404, message: "Group not found" } satisfies Partial<TripsApiError>);
  });
});
