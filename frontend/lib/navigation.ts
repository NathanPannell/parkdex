...
export function readNavigation(href: string): PublicNavigation {
  ...
  return {
    view: hasAccountCallback(url) ? "account" : view,
    selectedId: hasAccountCallback(url) ? null : selectedId,
    detailExpanded: params.get("detail") === "full",
...
const publicKeys = ["view", "place", "detail", "mode", "mapQuery", "mapCategory", "placesQuery", "placesCategory", "authority", "placesVisited"];
