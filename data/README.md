...
- Named regional parks from the CRD and RDN official GIS sources. RDMW coverage includes Bere Point, Kwaksistah, Little Huson Cave, and Mount Cain Alpine parks from official park pages and reports; each catalogue record credits the reviewed source used for its representative point, and named OpenStreetMap objects provide mobile boundary geometry where RDMW does not publish a geospatial feed. Features named as trails, including Morden Colliery Regional Trail, are excluded from this parks-only collection.
...
Place-specific official visitor pages are maintained separately in `frontend/lib/visitor-information.catalogue.json`, with the checked destination, authority, title, evidence URL and verification date. On 2026-09-12, the verified set covers all 117 provincial parks, both national reserves, 54 of 55 regional records and two of 24 islands. Sooke River Park and the remaining islands have no verified visitor link; their original place/boundary provenance remains available. The Salt Spring Island Chamber and Malcolm Island/Sointula regional destination organization supply the two verified island visitor guides. Links are not synthesized from park names and do not imply a verified entrance or booking integration.

Visitor-facing summaries are kept in `place-descriptions.catalogue.json` and `nonprovincial-descriptions.catalogue.json`. Each entry records its source page URL and title, source section, review date, and description status. `scripts/data-build.mjs` merges these records by stable place ID into `places.json`; it does not scrape or rewrite copy at build time. The 2026-09-23 review found useful overview content on 112 of 117 provincial BC Parks pages. Three more parks with no page overview now use relevant facts from other official provincial sources: Arbutus Grove's purpose statement, Hwsalu-Utsum's provincial announcement, and Wood Mountain's geographical names record. Beaver Point and Eves remain explicit `no-overview` gaps because no visitor-facing park description was found in authoritative sources. `scripts/data-validate.mjs` checks exact catalogue coverage, copied descriptions, source links, and these two gaps.

The official BC Parks page names `Háthayim Marine Park`, also known as `Von Donop Marine Park`: https://bcparks.ca/hathayim-marine-park-aka-von-donop-marine-park/. The 2026-09-12 display correction removes the source feed's unfinished bracketed alias while retaining source ID `728` and catalogue ID `provincial-hathayim-marine-park-a-k-a-von-donop-marine-park`. The importer correction is keyed by source identity; search accepts both names with or without accents. The catalogue delimiter audit found no other unmatched names, and data validation now checks balanced brackets and parentheses. Boundary coordinates are unchanged.

Regional coverage is strongest for CRD, RDN, CVRD, and the four named RDMW parks with defensible location evidence. The ACRD official list is represented by Mount Arrowsmith Regional Park through the RDN shared-boundary dataset, but China Creek and the linear trail properties lack a clean authoritative representative-point feed. SRD's directory labels its current facilities by types such as nature park, community park, beach access, and trail rather than exposing a stable regional-park set. Comox Valley established a regional parks service in 2022 and is still working through acquisition planning; its existing rural community parks are outside this regional-only category.

The current CRD GIS layer publishes the former Sooke River Regional Park as `Sooke River Park` with type `Municipal Park`. Parkdex keeps its established `regional-sooke-river-regional-park` ID while updating the displayed name, source object, and description so saved visits and group memberships stay attached.
...
Provincial and regional boundaries come from the same official DataBC, CRD, CVRD, and RDN layers used by the catalogue builder. Pacific Rim and Gulf Islands use Natural Resources Canada's Canada Lands Survey System legislative boundary service. Those polygons show the legislated reserve extent; they do not determine ownership, permitted access, or safe travel. The four RDMW parks use individually reviewed, named OpenStreetMap objects because RDMW does not publish a geospatial boundary feed.
